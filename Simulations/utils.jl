...
using Oceananigans.TurbulenceClosures: viscous_flux_ux, viscous_flux_uy, viscous_flux_uz,
                                       ...
                                       diffusive_flux_z, ∂ⱼ_τ₁ⱼ, ∂ⱼ_τ₂ⱼ, ∂ⱼ_τ₃ⱼ
...
@inline function get_time_invariant_fields(grid::RectilinearGrid)
    ...
    compute!(Bbak)
    compute!(Vbak)
    return Dict("us" => usla, "vs" => vsla, "dusdz" => dusdzla, "dvsdz" => dvsdzla,
                "Bbak" => Bbak, "Vbak" => Vbak)
end
...
@inline function SubgridscaleVerticalTracerFlux(model::NonhydrostaticModel, tracer_name; tracer=nothing)
    tracer_index = findfirst(n -> n === tracer_name, propertynames(model.tracers))
    tracer = tracer == nothing ? model.tracers[tracer_name] : tracer
    return KernelFunctionOperation{Center, Center, Face}(diffusive_flux_z, model.grid, model.closure,
